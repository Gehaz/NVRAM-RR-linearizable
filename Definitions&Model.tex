...
In order to formally capture this behavior we introduce a recover step, denoted $(RECOVER,p)$, represents the recovery of a process $p$, and the invocation of the \texttt{Recovery} function matching the pending operation, if there is one. A crash step is always followed by a recover step, and vice verse.
...
\begin{definition} [Recoverable response linearizability]
	...
	A finite history $H$ is recoverable response linearizable if:
	\begin{enumerate}
		\item $H$ contains operations to recoverable objects only;
		\item In $H$, every $(CRASH,p)$ step is either the last step of $p$, or that the next step by $p$ is $(RECOVER,p)$, and vice verse (every $(RECOVER,p)$ step is preceded by $(CRASH,p)$ step);
		\item If an invocation step has a matching response step, then it follows either the invocation step or a recover step;
		\item Let $H'$ be the history obtained from $H$ by removing all crash and recover steps. Then $H'$ is well-formed and linearizable.
	\end{enumerate}
\end{definition}
